...
class FinanceReport:
    ...
    def get_financial_report(self):
        page = requests.get(
            "https://finfo-api.vndirect.com.vn/v3/stocks/financialStatement?secCodes={"
            "}&reportTypes=QUARTER&modelTypes=1,89,101,411&fromDate={}&toDate={}".format(
    ...
    def get_business_report(self):
        page = requests.get(
            "https://finfo-api.vndirect.com.vn/v3/stocks/financialStatement?secCodes={"
            "}&reportTypes=QUARTER&modelTypes=2,90,102,412&fromDate={}&toDate={}".format(
                self.symbol, self.start, self.end))
        data = page.json()
        data_dates = {}
        for item in data['data']['hits']:
    ...
    def fetch_cash_flow_report(self):
        page = requests.get(
            "https://finfo-api.vndirect.com.vn/v3/stocks/financialStatement?secCodes={"
            "}&reportTypes=QUARTER&modelTypes=3,91,103,413&fromDate={}&toDate={}".format(
                self.symbol, self.start, self.end))
        data = page.json()
        data_dates = {}
    ...
    def get_basic_index(self):
        ...
        if len(years) > 1:
            ...
            user_agent = 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2272.101 ' \
                         'Safari/537.36 '
            headers = {'User-Agent': user_agent}
            for year in years:
                page = requests.get(
                    "https://finfo-api.vndirect.com.vn/v4/ratios?q=code:{}~itemCode:53030,52005,51050,53021,52001,52002,"
                    "54018,712010,712020,712030,712040~reportDate:{}".format(
